...
- **No build step, no framework, no dependencies.** `public/` is shipped as-is.
  ...
  `checkout.js → board.js → numpad.js → practice.js → trainer.js →
  scoring-stats.js → store.js → testlog.js → log.js → app.js`.
  Keep this order; `app.js` must load last (it boots everything on
  `DOMContentLoaded`).
- **PWA:** `manifest.webmanifest` + `icons/` (generated by the one-off
  `infra/make-icons.js`, pure Node, not part of deploy) + Apple `<meta>`/`<link>`
  tags make it installable to the iOS home screen (standalone, own icon).
...
- **Four views** (`#view-practice`, `#view-trainer`, `#view-chart`,
  `#view-log`) switched by the header tabs. `app.js` owns routing and a single
  global `keydown` bus that forwards to the active view's controller. The **Log**
  view (`log.js`) records power-scoring sessions via the optional backend
  (`Store` → Lambda/DynamoDB; see `infra/BACKEND.md`) and renders the trend with
  the `ScoringStats` math (`scoring-stats.js`, which has a Node export shim for
  unit tests). DynamoDB is the single source of truth for sessions; the
  `practice/` dir holds only the methodology (`practice/README.md`).
- **`testlog.js`** is a guided per-visit TEST entry **mode inside the Log tab**
  (not a routed view): it captures each 3-dart total into `visits[]`, autosaves
  to `localStorage`, and on confirm writes the session (darts/score derived from
  `visits` server-side). `log.js` delegates `onKey`/`onActivate` to it while
  active. **`Numpad` is a singleton** shared by Practice and TestLog — each
  re-`init`s it onto its own keypad container in `onActivate`/on entry, so it's
  always bound to the active view.
